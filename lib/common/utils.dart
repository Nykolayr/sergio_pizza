...
class Utils {
  ...
  static String hashPassword(String password) {
    final bytes = utf8.encode(password); // Преобразуем пароль в байты
    final digest = sha256.convert(bytes); // Хешируем байты
    return digest.toString(); // Возвращаем хеш в виде строки
  }

  /// высчитываем разницу между двумя датами вплоть до миллисекунд
  static Duration getDurationDifferent(DateTime date1, DateTime date2) {
  ...
  //     return langText.getText('Please enter your email adress');
  //   }
  //   final re = RegExp(r'.+@.+\..+');
  //   return (!re.hasMatch(value)) ? message : null;
  // }
